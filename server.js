...
db.getConnection((err, connection) => {
  ...
  console.log("Connected to MySQL database:", DB_NAME || "bunvic_tours");
  connection.release();
});
...
async function processAdminQueue() {
  ...
  while (!adminQueue.isEmpty()) {
    ...
    try {
      ...
      console.error("Admin queue job failed:", err?.message || err);
    }
  }
...
app.post("/api/bookings", (req, res) => {
  ...
  const sql = `
    INSERT INTO bookings (
      fullName, email, phone, country, adults, children,
      tourPackage, accommodation, vehicleType, pickup, dropoff,
      arrivalDate, arrivalTime, serviceType, specialRequests, status
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
  ...
  db.query(sql, values, (err) => {
    ...
    console.log(`Booking added: ${fullName} (${serviceType || "tour"})`);
...
// ===== GET BOOKINGS (Arrays: filter/search/sort/paginate) =====
app.get("/api/bookings", (req, res) => {
  ...
  db.query(sql, (err, results) => {
    ...
    if (q) {
      ...
      data = data.filter((b) =>
        String(b.fullName || "").toLowerCase().includes(query) ||
        String(b.email || "").toLowerCase().includes(query) ||
        String(b.phone || "").toLowerCase().includes(query) ||
        String(b.tourPackage || "").toLowerCase().includes(query)
      );
    }
    ...
    data.sort((a, b) => {
      const A = a[safeSortBy];
      const B = b[safeSortBy];

      if (safeSortBy === "arrivalDate") {
        const da = A ? new Date(A).getTime() : 0;
        const dbb = B ? new Date(B).getTime() : 0;
        return (da - dbb) * dir;
      }
...
app.patch("/api/bookings/:id/status", (req, res) => {
    ...
    db.query("SELECT * FROM bookings WHERE id = ?", [bookingId], (err, rows) => {
      ...
      db.query(
        ...
        (err2, result) => {
          ...
          console.log(`Booking ID ${bookingId} updated to: ${status}`);
          resolve();
        }
      );
    });
  ...
  return res.status(202).json({
    ...
    queueSize: adminQueue.size(),
  });
});

// ===== DELETE BOOKING (Queue + Stack undo) =====
...
// ===== UNDO LAST ADMIN ACTION (Stack -> Queue) =====
app.post("/api/admin/undo", (req, res) => {
  ...
  if (action.type === "STATUS_CHANGE") {
    adminQueue.enqueue(() => new Promise((resolve, reject) => {
      db.query(
        "UPDATE bookings SET status = ? WHERE id = ?",
        [action.previousStatus, action.bookingId],
        (err, result) => {
          if (err) return reject(err);
          if (result.affectedRows === 0) return reject(new Error("Booking not found"));
          console.log(`UNDO: Booking ${action.bookingId} status restored to ${action.previousStatus}`);
          resolve();
        }
      );
    }));
  ...
  if (action.type === "DELETE") {
    ...
    const values = [
      row.fullName, row.email, row.phone, row.country, row.adults, row.children,
      row.tourPackage, row.accommodation, row.vehicleType, row.pickup, row.dropoff,
      row.arrivalDate, row.arrivalTime, row.serviceType, row.specialRequests, row.status
    ];

    adminQueue.enqueue(() => new Promise((resolve, reject) => {
      db.query(sql, values, (err) => {
        if (err) return reject(err);
        console.log(`UNDO: Deleted booking restored (new id assigned).`);
        resolve();
      });
    }));

    processAdminQueue();
    return res.json({ success: true, message: "Undo queued: deleted booking restored (new id)." });
  }
...
app.get("/api/bookings/monthly", (req, res) => {
  const query = `
    SELECT 
      DATE_FORMAT(arrivalDate, '%b %Y') AS month,
      COUNT(*) AS total
    FROM bookings
    WHERE arrivalDate IS NOT NULL
    GROUP BY month
    ORDER BY MIN(arrivalDate);
  `;
...
app.use((req, res) => {
  res.sendFile(path.join(__dirname, "../bunvic-travel-car-hire/admin-login.html"));
});
